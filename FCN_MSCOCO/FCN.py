...
class FCN:
    ...
    def load_conv_weight(self, dict, name):
        init = tf.constant_initializer(value=dict[name][0], dtype=tf.float32)
        shape = dict[name][0].shape
        var = tf.get_variable(name=name, initializer=init, shape=shape)

        return var


    def load_bias_weight(self, dict, name):
        init = tf.constant_initializer(value=dict[name][1], dtype=tf.float32)
        shape = dict[name][1].shape
        return tf.get_variable(name=name+'_b', initializer=init, shape=shape)
    ...
    def build(self, input, label, keep_prob=0.5, is_training=False):
        ...
        W1 = self.load_conv_weight(vgg, 'conv1_1')
        L1 = tf.nn.conv2d(input, W1, strides=[1, 1, 1, 1], padding='SAME')
        b1 = self.load_bias_weight(vgg, 'conv1_1')
        L1 = tf.nn.bias_add(L1, b1)
        L1 = tf.nn.relu(L1)

        W2 = self.load_conv_weight(vgg, 'conv1_2')
        L2 = tf.nn.conv2d(L1, W2, strides=[1, 1, 1, 1], padding='SAME')
        b2 = self.load_bias_weight(vgg, 'conv1_2')
        ...
        W3 = self.load_conv_weight(vgg, 'conv2_1')
        L3 = tf.nn.conv2d(pool1, W3, strides=[1, 1, 1, 1], padding='SAME')
        b3 = self.load_bias_weight(vgg, 'conv2_1')
        L3 = tf.nn.bias_add(L3, b3)
        L3 = tf.layers.batch_normalization(L3, training=is_training)
        L3 = tf.nn.relu(L3)

        W4 = self.load_conv_weight(vgg, 'conv2_2')
        L4 = tf.nn.conv2d(L3, W4, strides=[1, 1, 1, 1], padding='SAME')
        b4 = self.load_bias_weight(vgg, 'conv2_2')
        ...
        #############################1x1 conv########################
        W14 = tf.Variable(tf.random_normal([7, 7, 512, 4096], stddev=0.01), dtype=np.float32)
        FCL1 = tf.nn.conv2d(pool5, W14, strides=[1, 1, 1, 1], padding='SAME')
        b14 = tf.Variable(tf.random_normal([4096], stddev=0.01), dtype=np.float32)
        ...
        b15 = tf.Variable(tf.random_normal([4096], stddev=0.01), dtype=np.float32)
        ...
        b16 = tf.Variable(tf.random_normal([label.shape[3].value], stddev=0.01), dtype=np.float32)
        ...
        ###########업샘플 1
        W17 = tf.Variable(tf.random_normal([4, 4, pool4.shape[3].value, label.shape[3].value], stddev=0.01),
                          dtype=np.float32)
        UCL1 = tf.nn.conv2d_transpose(FCL3, W17, output_shape=[self.input_size, pool4.shape[1].value, pool4.shape[2].value,
                                                               pool4.shape[3].value], strides=[1, 2, 2, 1],
                                      padding='SAME')
        b17 = tf.Variable(tf.random_normal([pool4.shape[3].value], stddev=0.01), dtype=np.float32)
        UCL1 = tf.nn.bias_add(UCL1, b17)
        UCL1 = tf.layers.batch_normalization(UCL1, training=is_training)
        UCL1 = tf.nn.relu(UCL1)

        ###########skip 연결 from pool4 + 업샘플 1
        UCL2 = tf.add(UCL1, pool4)

        ###########업샘플 2
        W18 = tf.Variable(tf.random_normal([4, 4, pool3.shape[3].value, UCL2.shape[3].value], stddev=0.01),
                          dtype=np.float32)
        UCL3 = tf.nn.conv2d_transpose(UCL2, W18, output_shape=[self.input_size, pool3.shape[1].value, pool3.shape[2].value,
                                                               pool3.shape[3].value], strides=[1, 2, 2, 1],
                                      padding='SAME')
        b18 = tf.Variable(tf.random_normal([pool3.shape[3].value], stddev=0.01), dtype=np.float32)
        UCL3 = tf.nn.bias_add(UCL3, b18)

        ###########skip 연결 from pool3 + 업샘플 2
        UCL4 = tf.add(UCL3, pool3)

        ###########업샘플 3
        W19 = tf.Variable(tf.random_normal([16, 16, label.shape[3].value, UCL4.shape[3].value], stddev=0.01),
                          dtype=np.float32)
        UCL5 = tf.nn.conv2d_transpose(UCL4, W19, output_shape=[self.input_size, label.shape[1].value, label.shape[2].value,
                                      ...
                                      padding='SAME')
        b19 = tf.Variable(tf.random_normal([label.shape[3].value], stddev=0.01), dtype=np.float32)
    ...
    def run(self, max_iter, already_done):
        ...
        input_batch, label_batch = img_loader.nextbatch(self.input_size, 0, stochastic=True)
        ...
        for epoch in range(max_iter) :
            for itr in range(batch_num):
                input_batch, label_batch = img_loader.nextbatch(self.input_size, itr, stochastic=True)
